<?php
    ...
    $user = 'root'; 
    $password = '010523'; 
    $dbname = 'CIS_105_Lab10';

    $conn = mysqli_connect($host, $user, $password, $dbname); // Replace this with your actual connection code
    if(!$conn){
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['create_order'])) {
            ...
            $toppings = isset($_POST['topping_ids']) ? $_POST['topping_ids'] : [];
            $qty = $_POST['qty'];
            ...
            if ($pizza_result && mysqli_num_rows($pizza_result) > 0) {
                ...
                $pizza_str = (string)$pizza_id;
                $toppings_str = implode(",", $toppings);

                $stmt = mysqli_prepare($conn, 
                "INSERT INTO orders (customer, pizza, toppings, qty, total, status) VALUES (?, ?, ?, ?, ?, 'Pending')");

                mysqli_stmt_bind_param($stmt, "sssid", $customer, $pizza_str, $toppings_str, $qty, $grand_total);
                    ...
                    <tbody>
                        <?php
                            ...
                            if ($res && mysqli_num_rows($res) > 0) {
                                ...
                                echo "<tr><td colspan='3' style='text-align:center;'><em>No pizzas found. Add some delicious options!</em></td></tr>";
                            }
                    ...
                    <tbody>
                        <?php
                            ...
                            while($row = mysqli_fetch_assoc($res)) {
                                ...
                                $details = "Pizza ID: $pizza | Toppings: $toppings | Qty: $qty";
